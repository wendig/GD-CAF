...
class UNetDS_Attention(Precipitation_base):
    def __init__(self, hparams):
        ...
        self.inc = DoubleConvDS(self.n_channels, self.n_channels, kernels_per_layer=kernels_per_layer)
        self.cbam1 = CBAM(self.n_channels, reduction_ratio=reduction_ratio)
        self.down1 = DownDS(self.n_channels, self.n_channels*2, kernels_per_layer=kernels_per_layer)    # 64
        self.cbam2 = CBAM(self.n_channels*2, reduction_ratio=reduction_ratio)
        self.down2 = DownDS(self.n_channels*2, self.n_channels*4, kernels_per_layer=kernels_per_layer)
        self.cbam3 = CBAM(self.n_channels*4, reduction_ratio=reduction_ratio)
        self.down3 = DownDS(self.n_channels*4, self.n_channels*8, kernels_per_layer=kernels_per_layer)
        self.cbam4 = CBAM(self.n_channels*8, reduction_ratio=reduction_ratio)
        factor = 2 if self.bilinear else 1
        self.down4 = DownDS(self.n_channels*8, self.n_channels*16 // factor, kernels_per_layer=kernels_per_layer)
        self.cbam5 = CBAM(self.n_channels*16 // factor, reduction_ratio=reduction_ratio)
        self.up1 = UpDS(self.n_channels*16, self.n_channels*8 // factor, self.bilinear, kernels_per_layer=kernels_per_layer)
        self.up2 = UpDS(self.n_channels*8, self.n_channels*4 // factor, self.bilinear, kernels_per_layer=kernels_per_layer)
        self.up3 = UpDS(self.n_channels*4, self.n_channels*2 // factor, self.bilinear, kernels_per_layer=kernels_per_layer)
        self.up4 = UpDS(self.n_channels*2, self.n_channels, self.bilinear, kernels_per_layer=kernels_per_layer)
    ...
    def forward(self, x):
        ...
        #print(f'self.up3: {x.size()}')
        #print(f'x1Att: {x1Att.size()}')
